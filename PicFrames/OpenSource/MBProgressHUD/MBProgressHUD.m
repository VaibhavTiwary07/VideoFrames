...
- (void)updateProgress {
    [(local_MBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[local_MBRoundProgressView alloc] init];
#else
        self.indicator = [[[local_MBRoundProgressView alloc] init] autorelease];
#endif
}
...
@implementation local_MBRoundProgressView
