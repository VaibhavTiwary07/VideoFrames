...
+(CGRect)getTheFrameSize
{
    ...
    float size = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)?700.0:300.0;
    float y = (fullscreen.size.height - size)/2.0;
    float x = (fullscreen.size.width - size)/2.0;
    
    defaultFrameSize = CGRectMake(x, y, size, size);
#if 0
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
    {
        defaultFrameSize = CGRectMake(34.0, y, 700.0, 700.0);
    }
    else
    {
        defaultFrameSize = CGRectMake(10.0, y, 300.0, 300.0);
    }
#endif
...
-(UIImage*)quickRenderToImageOfScale:(float)scale
{
    ...
    for(index = 0; index < self.photoCount; index++)
    {
        ...
        if(nil != pht)
        {
            //pht.view.backgroundColor = [UIColor clearColor];
...
-(void)setInnerRadius:(int)radius
{
    ...
    for(iIndex = 0; iIndex < [photos count]; iIndex++)
    {
        ...
        if(nil != pht)
        {
            //NSLog(@"setInnerRadius for photo %d %d",iIndex,radius);
            pht.view.layer.masksToBounds = YES;
