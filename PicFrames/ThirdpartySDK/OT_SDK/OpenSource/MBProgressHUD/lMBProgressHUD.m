...
#import "lMBProgressHUD.h"

@interface lMBProgressHUD ()
...
+ (lMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	lMBProgressHUD *hud = [[lMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[lMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		lMBProgressHUD *HUD = (lMBProgressHUD *)viewToRemove;
