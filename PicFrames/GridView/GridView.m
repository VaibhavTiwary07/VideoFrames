...
-(void)patternSelected:(id)sender
{
    ...
    {
        [delegate itemSelectedAtIndex:(int)btn.tag ofGridView:self];
    }
...
+(UIColor *)getColorFromHexAtIndex:(int)hexint
{
    return [UIColor colorWithHex:hexint];
}
...
{
    NSString *pFileName = nil;
    pFileName = [NSString stringWithFormat:@"pattern_%d",index];
    //NSLog(@"pattern file name %@",pFileName);
    ...
    NSLog(@"pattern color at index %d is %@ ",index,pattern);
    return pattern;
}

+(UIImage *)getPatternImageAtIndex:(int)index
{
    NSString *pFileName = nil;
    pFileName = [NSString stringWithFormat:@"pattern_%d",index];
    //NSLog(@"pattern file name %@",pFileName);
...
+(UIColor *)getColorOfImageAtIndex:(int)index imagePrefix:(NSString*)prefix
{
    NSString *pFileName = nil;
    pFileName = [NSString stringWithFormat:@"%@%d",prefix,index];
    //NSLog(@"pattern file name %@",pFileName);
    [pFileName stringByAppendingString:@".png"];
    UIImage *colorImage = [UIImage imageNamed:pFileName];
    UIColor *color = [UIColor colorWithPatternImage:colorImage];
   // NSLog(@"pattern color at index %d is %@ ",index,color);
    return color;
}

+(UIImage *)getImageAtIndex:(int)index imagePrefix:(NSString*)prefix
{
    NSString *pFileName = nil;
    pFileName = [NSString stringWithFormat:@"%@%d",prefix,index];
   // NSLog(@"pattern file name %@",pFileName);
...
+(UIColor *)getColorForImage:(UIImage*)image
{
    NSLog(@"image size %f, image width %f",image.size.width,image.size.height);
    NSLog(@"image size %f, image width %f",image.size.width,image.size.height);
...
-(void)loadPage:(int)pageNum
{
    ...
    {
        NSLog(@" grid view loadPage: invalid pagenumber %d for pagecount %d",pageNum,_pageCount);
        return;
    }
