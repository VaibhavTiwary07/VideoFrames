...
-(void)uploadToFacebook
{
    ...
    [[OT_Facebook SharedInstance]uploadImage:[self getTheSnapshot] withMessage:@"Uploaded from VideoCollage Free iphone app. get it from http://www.videocollageapp.com" onCompletion:^(BOOL uploadStatus){
        [Utility removeActivityIndicatorFrom:view];
    }];
...
- (NSString *)uploadImageToOurServer:(UIImage*)image
{
	...
	NSLog(@"uploadImageToOurServer result: %@",returnString);
...
-(void)uploadToUserWallInMainThread
{
    ...
    */
    [[OT_Facebook SharedInstance]postOnWallWithImage:[self getTheSnapshot] message:@"Uploaded from VideoCollage Free iPhone App" website:@"http://www.videocollageapp.com" onCompletion:^(BOOL uploadStatus){
       
        if(uploadStatus)
        {
            NSLog(@"Successfully uploaded to facebook wall");
        }
        else
        {
            NSLog(@"Failed to upload to facebook wall");
        }
    }];
    return;
}
