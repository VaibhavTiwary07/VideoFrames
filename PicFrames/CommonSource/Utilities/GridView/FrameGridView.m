...
-(void)frameSelected:(id)sender
{
    ...
    {
        [delegate frameSelectedAtIndex:btn.tag ofGridView:self];
    }
...
- (void) buttonPress:(UIButton*)button 
{
    ...
    {
        if(NO == [self.delegate gridView:self contentLockedAtIndex:button.tag])
        {
            ...
            [button setBackgroundImage:[self coloredImageForItem:button.tag] forState:UIControlStateNormal];
        }
    }
...
-(UIView*)allocPageForPageNume:(int)pageNum
{
    ...
    for(i = 0; i < _rows;i++)
    {
        for(j = 0; j < _cols;j++)
        {
            ...
            if([self.delegate respondsToSelector:@selector(gridView:contentLockedAtIndex:)])
            {
                if([self.delegate gridView:self contentLockedAtIndex:btn.tag])
                {
                    if(UIUserInterfaceIdiomPad == UI_USER_INTERFACE_IDIOM())
                    {
                        [btn setImage:[UIImage imageNamed:@"lock_corner_ipad.png"] forState:UIControlStateNormal];
                        //[btn setBackgroundImage:[UIImage imageNamed:@"lock_corner_ipad.png"] forState:UIControlStateNormal];
                    }
                    else
                    {
                        [btn setImage:[UIImage imageNamed:@"lock_corner.png"] forState:UIControlStateNormal];
                        //[btn setBackgroundImage:[UIImage imageNamed:@"lock_corner.png"] forState:UIControlStateNormal];
                    }
                }
            }
            else{
