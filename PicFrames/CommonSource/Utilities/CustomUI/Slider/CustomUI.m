...
+ (UISlider *)allocateCustomSlider:(CGRect)rect {
    ...
    // Load the images
    UIImage *fillImage = [UIImage imageNamed:@"fill@2x"];
    UIImage *trackImage = [UIImage imageNamed:@"track@2x"];
    
    // Create stretchable images using modern methods
    UIImage *stretchableFillImage = [fillImage resizableImageWithCapInsets:UIEdgeInsetsMake(0.0f, 6.0f, 0.0f, 6.0f)];
    UIImage *stretchableTrackImage = [trackImage resizableImageWithCapInsets:UIEdgeInsetsMake(0.0f, 6.0f, 0.0f, 6.0f)];

    // Set track images
    [slider setMinimumTrackImage:stretchableFillImage forState:UIControlStateNormal];
    [slider setMaximumTrackImage:stretchableTrackImage forState:UIControlStateNormal];

    // Use a placeholder or default thumb image if available
    UIImage *thumbImage = [UIImage systemImageNamed:@"circle.fill"]; // Replace with your thumb image name
    if (thumbImage) { //Modified by Sachin
