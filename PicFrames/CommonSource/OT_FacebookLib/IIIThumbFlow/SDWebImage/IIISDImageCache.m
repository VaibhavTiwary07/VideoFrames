...
static IIISDImageCache *instance;

static NSInteger cacheMaxCacheAge = 60*60*24*7; // 1 week

@implementation IIISDImageCache
...
#pragma mark IIISDImageCache (class methods)

+ (IIISDImageCache *)sharedImageCache
{
    if (instance == nil)
    {
        instance = [[IIISDImageCache alloc] init];
    }
...
#pragma mark IIISDImageCache (private)
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    id <IIISDImageCacheDelegate> delegate = [arguments objectForKey:@"delegate"];
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <IIISDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    if (!delegate)
    {
        return;
