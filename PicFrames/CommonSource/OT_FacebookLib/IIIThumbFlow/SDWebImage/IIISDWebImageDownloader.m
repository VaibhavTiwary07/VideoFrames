...
#import "IIISDWebImageDecoder.h"
#import <ImageIO/ImageIO.h>

@interface IIISDWebImageDownloader (ImageDecoder) <IIISDWebImageDecoderDelegate>
@end

NSString *const IIISDWebImageDownloadStartNotification = @"IIISDWebImageDownloadStartNotification";
NSString *const IIISDWebImageDownloadStopNotification = @"IIISDWebImageDownloadStopNotification";

@interface IIISDWebImageDownloader ()
@property (nonatomic, retain) NSURLConnection *connection;
@end

@implementation IIISDWebImageDownloader
...
+ (id)downloaderWithURL:(NSURL *)url delegate:(id<IIISDWebImageDownloaderDelegate>)delegate userInfo:(id)userInfo lowPriority:(BOOL)lowPriority
{
    ...
    // To use it, just add #import "SDNetworkActivityIndicator.h" in addition to the IIISDWebImage import
...
#pragma clang diagnostic pop
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:activityIndicator name:IIISDWebImageDownloadStartNotification object:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:activityIndicator name:IIISDWebImageDownloadStopNotification object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"startActivity")
                                                     name:IIISDWebImageDownloadStartNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:activityIndicator
                                                 selector:NSSelectorFromString(@"stopActivity")
                                                     name:IIISDWebImageDownloadStopNotification object:nil];
    }

    IIISDWebImageDownloader *downloader = SDWIReturnAutoreleased([[IIISDWebImageDownloader alloc] init]);
    downloader.url = url;
...
- (void)start
{
    // In order to prevent from potential duplicate caching (NSURLCache + IIISDImageCache) we disable the cache for image requests
    ...
    if (connection)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:IIISDWebImageDownloadStartNotification object:self];
    }
    else
    {
...
- (void)cancel
{
    if (connection)
    {
        [connection cancel];
        self.connection = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:IIISDWebImageDownloadStopNotification object:self];
    }
}
...
- (void)connection:(NSURLConnection *)aConnection didReceiveResponse:(NSURLResponse *)response
{
    ...
    else
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:IIISDWebImageDownloadStopNotification object:nil];
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
        ...
        if (width + height > 0 && totalSize < expectedSize)
        {
            ...
            if (partialImageRef)
            {
                ...
                [[IIISDWebImageDecoder sharedImageDecoder] decodeImage:image
                                                       withDelegate:self
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    ...
    if ([delegate respondsToSelector:@selector(imageDownloader:didFinishWithImage:)])
    {
        ...
        [[IIISDWebImageDecoder sharedImageDecoder] decodeImage:image withDelegate:self userInfo:nil];
    }
}
...
#pragma mark IIISDWebImageDecoderDelegate

- (void)imageDecoder:(IIISDWebImageDecoder *)decoder didFinishDecodingImage:(UIImage *)image userInfo:(NSDictionary *)aUserInfo
{
