...
@implementation IIISDWebImageDecoder
static IIISDWebImageDecoder *sharedInstance;
...
- (void)notifyDelegateOnMainThreadWithInfo:(NSDictionary *)dict
{
    ...
    id <IIISDWebImageDecoderDelegate> delegate = [decodeInfo objectForKey:DELEGATE_KEY];
...
+ (IIISDWebImageDecoder *)sharedImageDecoder
{
    if (!sharedInstance)
    {
        sharedInstance = [[IIISDWebImageDecoder alloc] init];
    }
