...
#import "IIISDImageCache+IIIThumb.h"
...
/*
 * IIISDWebImage provides an approach to cache images (in both memory and disk layers). 
 * That's really a good job. For more information, see: 
 * https://github.com/rs/IIISDWebImage/wiki/How-is-IIISDWebImage-better-than-X%3F
 *
...
- (BOOL)loadCell:(IIIFlowCell *)cell AtIndex:(int)index {
    ...
    if (!thumb) {
        ...
        oImg = [[IIISDImageCache sharedThumbImageCache] imageFromKey:url];
        if (!oImg) {
            ...
            if (d.local_url.length) {
                ...
                if (!oImg) {
                    ...
                    [[IIISDImageCache sharedThumbImageCache] storeImage:oImg forKey:url];
                }
            } else {
                // If it's a web url, asynchronize download image, suspend loading cells
                if (![_downloadingURLs objectForKey:d.web_url]) {
                    [IIISDWebImageDownloader downloaderWithURL:[NSURL URLWithString:d.web_url] delegate:self userInfo:[NSDictionary dictionaryWithObject:[NSNumber numberWithInt:index] forKey:INDEX_KEY] lowPriority:YES];
...
#pragma mark - IIISDWebImageDownloaderDelegate methods
- (void)imageDownloader:(IIISDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image {
    ...
    if ([[NSString stringWithFormat:@"%i", index] isEqualToString:[_downloadingURLs objectForKey:urlStr]]) {
        ...
        [[IIISDImageCache sharedThumbImageCache] storeImage:image forKey:urlStr];
