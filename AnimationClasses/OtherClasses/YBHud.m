...
-(void)showInView:(UIView *)view animated:(BOOL)shouldAnimate{
  ...
  //  UIColor *hudColor = (_hudColor == nil) ? [UIColor blackColor] : _hudColor;  //changed to clear
    UIColor *hudColor = (_hudColor == nil) ? [UIColor clearColor] : _hudColor;
    if(_dimAmount == 0){ _dimAmount = 0.0; }
   ...
   // indicator.center = blockView.center; //changed here//
    indicator.center = CGPointMake(blockView.frame.size.width/2, blockView.frame.size.height/2+blockView.frame.size.height/4);
